...
bool isWinner(bool isPlayer1, int lastCol) {
  ...
  for (int r = ROWS-1; r >= 0; r--) {
    if (board[lastCol][r] == currTile) {
      counter++;
      if (counter == 4) {
        return true; //return true if winner
      }
  ...
  for (int r = ROWS-1; r >= 0; r--) {
    for (int c = 0; c<COLS-3;c++){
      if ((board[c][r] == currTile) && (board[c+1][r] == currTile) &&
       (board[c+2][r] == currTile) && (board[c+3][r] == currTile)){
        return true;//return true if winner
      }
    }
  }
  ...
  for (int r = ROWS-1; r >= 0; r--) {
    for (int c = 0; c<COLS-3;c++){
      if ((board[c][r+3] == currTile) && (board[c+1][r+2] == currTile) &&
       (board[c+2][r+1] == currTile) && (board[c+3][r] == currTile)){
        return true;//return true if winner
      }
    }
  }
